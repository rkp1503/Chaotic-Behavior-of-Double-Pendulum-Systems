...
\figref{fig:1} captures a frame of the double pendulum. Here we will set the initial anchor at the origin $O$. $x_i$ represents the horizontal distance from the origin to bob $i$, $y_i$ represents the vertical distance from the origin to bob $i$, and $\theta_i$ represents the angle bob $i$ makes between the anchor or the bob above it and the vertical. \\

We can derive the equations that are necessary to show the behavior of this chaotic system. Let's start by deriving the potential, kinetic, and the total energy of this system. From \figref{fig:1}, we can determine the positions of each bob. The $x$ and $y$ coordinates for bob 1 are:
